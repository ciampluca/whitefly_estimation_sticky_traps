...
def draw_groundtruth_and_predictions(image, gp, radius=10, marker='circle', palette=None, only_gt=False, only_pred=False, image_id=None):
    ...
    if only_gt:
        # iterate gt
        gt_counter = 0
        for c_gt, r_gt in gp[['X', 'Y']].values:
            ...
            if has_gt:  # draw groundtruth
                ...
                draw.set_color(image, (rr, cc), color, alpha=val)
        print("Image: {}, GT: {}".format(image_id, gt_counter))
    elif only_pred:
        # iterate predictions
        pred_counter = 0
        for c_p, r_p in gp[['Xp', 'Yp']].values:
            ...
            if has_p:  # draw prediction
                ...
                draw.set_color(image, (rr, cc), color, alpha=val)
        print("Image: {}, Det: {}".format(image_id, pred_counter))
    elif not only_gt and not only_pred:
