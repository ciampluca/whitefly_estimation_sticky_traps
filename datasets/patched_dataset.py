...
class PatchedImageDataset(Dataset):
    ...
    def __init__(
        ...
        Args:
            ...
            as_gray (bool, optional): Whether to load data as grayscale image. Defaults to False.
        ...
        # Retrieve image size without opening it
        ...
        self.patch_hw = (patch_size, patch_size) if patch_size else data_shape
    ...
    def _get_datum(self, index):
        ...
        if self.in_memory:
            ...
            data = io.imread(self.path, as_gray=self.as_gray).astype(np.float32)
            data *= 255. if self.as_gray else 1.  # gray values are stored between 0 and 1 by skimage's imread
            patch = data[sy:ey, sx:ex] / np.array(255., dtype=np.float32)
        if patch.ndim == 2:
