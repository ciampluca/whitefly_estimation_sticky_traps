...
def _save_image_with_boxes(
    ...
    class_labels = np.unique(gt_labels)
    for i in class_labels:
        ...
        # Add text to image
        text = f"Det N. Insects (Cls#{i}): {len(det_boxes)}, GT N. Insects (Cls#{i}): {len(gt_boxes)}"
    ...
    for n_i, (image_id, image_hw, image, boxes, labels, scores) in enumerate(progress):
        ...
        if outdir and debug:
            ...
            for i in range(n_classes):
                ...
                #image = draw_groundtruth_and_predictions(image, gp_i, radius=radius, marker='square', only_pred=True, image_id=image_id)
                io.imsave(outdir / f'annot_cls{i}_{image_id}', image)
            
            #image = draw_groundtruth_and_predictions(image, gp, radius=radius, marker='circle')
